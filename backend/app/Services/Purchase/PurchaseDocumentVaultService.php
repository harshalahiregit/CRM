...
/**
 * Unified Purchase Document Vault — the Purchase-side mirror of
 * TpvDocumentVaultService (parity rule). Read-only aggregator over the Purchase
 * document stores — statutory vendor documents, worker documents, and the
 * closure evidence attached to CAPAs and NCRs — normalised to one row shape with
 * a computed expiry state (Rule 8). Purely additive; it never writes.
 */
class PurchaseDocumentVaultService
{
    public const EXPIRING_WINDOW_DAYS = 30;

    public const SOURCES = ['Statutory', 'Worker', 'CAPA', 'NCR'];
    ...
    public function roster(int $tenantId, array $filters = []): array
    {
        ...
        $rows = array_merge(
            ...
            $this->workerDocs($tenantId, $vendorId),
    ...
    private function workerDocs(int $tenantId, ?int $vendorId): array
    {
        return PurchaseWorkerDocument::forTenant($tenantId)
            ->when($vendorId, fn ($q) => $q->where('purchase_vendor_id', $vendorId))
            ->with('vendor:id,company_name,purchase_vendor_code')
            ->get()
            ->map(fn (PurchaseWorkerDocument $d) => $this->row(
                source: 'Worker',
                id: $d->id,
                reference: $d->original_name ?? $d->type,
                title: ucwords(str_replace('_', ' ', (string) $d->type)),
                vendor: $d->vendor,
                file: $d->file_path,
                status: $d->status,
                uploaderId: $d->created_by,
                uploadedAt: $d->created_at,
                expiresAt: $d->expires_at,
            ))->all();
    }
    ...
    {
        $model = $kind === 'CAPA' ? PurchaseCapa::class : PurchaseNcr::class;
    ...
    private function fillUploaderNames(array &$rows): void
    {
