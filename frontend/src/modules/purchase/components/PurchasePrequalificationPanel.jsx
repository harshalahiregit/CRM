...
/**
 * Purchase vendor Prequalification — the Purchase-side mirror of the TPV
 * VendorPrequalificationPanel.
 *
 * A scored, sectioned questionnaire → Qualified / Conditional / Not Qualified
 * (the spec's 82/100 bar). Higher is better — the opposite polarity to the lean
 * admin-set Risk Score. Shows the current outcome and, for managers, an
 * assessment form with a live score/outcome preview that mirrors the backend
 * scoring in PurchasePrequalificationService. 100% Purchase-owned: hits only
 * /api/purchase/vendors/*, never TPV.
 */
const STATUS_CFG = {
...
export function PurchasePrequalificationPanel({ vendorId, manage }) {
  ...
  const load = () => {
    ...
    purchaseApi.vendors.prequalification(vendorId)
  ...
  // Live preview — mirrors PurchasePrequalificationService::compute()/fromScore().
  ...
  const save = async () => {
    ...
    try {
      const d = await purchaseApi.vendors.assessPrequalification(vendorId, { answers: ans, notes: notes || undefined })
...
export default PurchasePrequalificationPanel
