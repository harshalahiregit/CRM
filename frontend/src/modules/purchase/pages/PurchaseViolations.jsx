...
// the TPV engine (parity). Warning → Strike 1/2/3 → Suspension → Blacklist, from
// cumulative open-violation points. Enforcement runs through PurchaseVendorService.
...
export default function PurchaseViolations() {
  const { user } = useAuth()
  ...
  const remove = async (id) => { if (window.confirm('Delete this violation?')) { await purchaseApi.violations.delete(id); load() } }
  const closeV = async (v) => { await purchaseApi.violations.update(v.id, { status: 'Closed' }); load() }
  const enforce = async (row, action) => {
    ...
    try { await purchaseApi.violations.enforce(row.vendor_id, { action, reason }); load() }
            ...
            <div key={e.vendor_id} className="pr-glass" style={{ padding: 14, borderRadius: 13, outline: (e.level === 'Blacklist' || e.level === 'Suspension') ? `1.5px solid ${LEVEL_TONE[e.level]}66` : 'none' }}>
              ...
              {admin && (e.level === 'Suspension' || e.level === 'Blacklist') && e.vendor_status !== 'Blacklisted' && (
                <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                  {e.vendor_status !== 'On_Hold' && <button onClick={() => enforce(e, 'suspend')} style={{ ...miniBtn, color: '#f59e0b' }}><PauseCircle size={13} /> Suspend</button>}
              ...
              {loadError ? <tr><td colSpan={7} style={{ padding: 8 }}><LoadError error={loadError} onRetry={load} /></td></tr>
                ...
                : rows.map(v => (
                  <tr key={v.id} style={{ borderTop: '1px solid var(--border)' }}>
                    <td style={{ padding: '10px 14px', fontWeight: 700, color: '#38bdf8' }}>{v.reference}</td>
...
function RecordModal({ types, severities, vendors, onClose, onSaved }) {
  const [form, setForm] = useState({ purchase_vendor_id: '', type: types[0] || 'Quality_Failure', severity: 'Minor', occurred_at: '', description: '' })
  ...
  const save = async () => {
    ...
    try {
      ...
      await purchaseApi.violations.record(payload); onSaved()
    ...
    <div style={overlay} onClick={onClose}>
      <div className="pr-glass" style={sheet} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>
          ...
          <label style={lbl}>Type *
            <select value={form.type} onChange={set('type')} style={inp}>
              {(types.length ? types : ['Quality_Failure']).map(t => <option key={t} value={t}>{fmt(t)}</option>)}
            </select>
          </label>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16 }}>
          ...
          <button onClick={save} disabled={saving || !form.purchase_vendor_id} style={{ ...btnPrimary, opacity: (saving || !form.purchase_vendor_id) ? 0.6 : 1 }}>{saving ? 'Saving…' : 'Record'}</button>
        </div>
      </div>
    </div>
  )
