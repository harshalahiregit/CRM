...
// Purchase Non-Conformance Reports — the Purchase-side mirror of the TPV register
// (parity rule). Raised → Assigned → Response → Corrective Action → Verification → Closed.
...
export default function PurchaseNcr() {
  const { user } = useAuth()
  ...
  const advance = async (row, status) => {
    try { await purchaseApi.ncrs.transition(row.id, { status }); load() }
    catch (e) { alert(e?.response?.data?.message || 'Could not update.') }
  }
  const remove = async (id) => { if (window.confirm('Delete this NCR?')) { await purchaseApi.ncrs.delete(id); load() } }
                ...
                : rows.map(n => {
                  ...
                  return (
                    <tr key={n.id} style={{ borderTop: '1px solid var(--border)' }}>
                      <td style={{ padding: '10px 14px', fontWeight: 700, color: '#38bdf8' }}>{n.reference}</td>
...
function NcrModal({ row, severities, vendors, onClose, onSaved }) {
  ...
  const seed = () => {
    const b = { purchase_vendor_id: row?.purchase_vendor_id || '', severity: row?.severity || 'Major', due_date: row?.due_date ? String(row.due_date).slice(0, 10) : '' }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (row) await purchaseApi.ncrs.update(row.id, payload)
      else await purchaseApi.ncrs.create(payload)
      onSaved()
