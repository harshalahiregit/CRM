...
// Purchase Communications Centre — the Purchase-side mirror of the TPV centre
// (parity rule). A derived alerts feed + send-to-vendor over email/WhatsApp/SMS,
// logged. Alert kinds whose Purchase mirrors haven't landed are simply absent.
const SEV_TONE = { high: '#ef4444', medium: '#f59e0b', low: '#0ea5e9' }
const KIND_LABEL = { document_expiry: 'Document', ncr_overdue: 'NCR', capa_overdue: 'CAPA' }
const date = (d) => (d ? new Date(d).toLocaleDateString() : '—')

export default function PurchaseCommunications() {
  const { user } = useAuth()
  ...
  const load = useCallback(() => {
    purchaseApi.communications.get().then(setData).catch(() => setData({ alerts: [], log: [], channels: [] }))
  }, [])
  useEffect(() => { load() }, [load])
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr)', gap: 16, alignItems: 'start' }}>
        <div className="pr-glass" style={{ padding: 0, borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ padding: '14px 16px', fontSize: 15, fontWeight: 800, color: 'var(--text-h)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <Megaphone size={16} style={{ color: '#38bdf8' }} /> Action feed <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)' }}>· {alerts.length}</span>
          </div>
...
function ComposeModal({ seed, channels, onClose, onSent }) {
  ...
  const send = async () => {
    ...
    try {
      await purchaseApi.communications.send({ vendor_id: seed.vendor_id, ...form })
      onSent()
...
const miniBtn = { display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-card)', color: '#38bdf8', cursor: 'pointer', fontSize: 11.5, fontWeight: 700 }
