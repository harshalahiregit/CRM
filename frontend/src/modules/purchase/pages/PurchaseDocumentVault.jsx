...
import { RefreshCw, Search, FileText, ExternalLink } from 'lucide-react'
import { purchaseApi } from '@/services/purchaseApi'
import LoadError from '@/components/ui/LoadError'
import { KIT3D_STYLE as PURCHASE_STYLE } from '@/components/ui/kit3d'

// Purchase Document Vault — the Purchase-side mirror of the TPV vault (parity).
// Read-only lens over statutory vendor documents, worker documents, and CAPA/NCR
// closure evidence, with a computed expiry state so nothing lapses unseen.
const SOURCE_TONE = { Statutory: '#0ea5e9', Worker: '#8b5cf6', CAPA: '#22c55e', NCR: '#ef4444' }
...
export default function PurchaseDocumentVault() {
  ...
  const load = useCallback(() => {
    ...
    if (q) params.q = q
    purchaseApi.documentVault.list(params)
  ...
  const cards = [
    { k: 'Documents', v: sum.total ?? 0, c: '#38bdf8', filter: () => { setSourceF(''); setExpiryF('') } },
    ...
    <div style={{ padding: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 12.5, margin: '4px 0 0' }}>Every vendor document — statutory, worker, CAPA &amp; NCR — in one lens with expiry tracking.</p>
        </div>
...
const iconBtn = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: 6, borderRadius: 8, border: 'none', background: 'transparent', color: '#38bdf8', cursor: 'pointer', textDecoration: 'none' }
