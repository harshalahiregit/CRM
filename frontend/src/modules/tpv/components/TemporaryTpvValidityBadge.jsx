...
/**
 * Remaining-validity badge for a Third-Party Vendor.
 *
 * TPV-owned: no Purchase import, no shared business logic (Purchase has its own
 * TemporaryVendorValidityBadge). Takes the server's `validity_countdown` payload
...
export function tpvValidityLabel(countdown, nowMs = Date.now()) {
...
export default function TemporaryTpvValidityBadge({ countdown, compact = false, showLabel = false }) {
  ...
  const { label, tone } = tpvValidityLabel(countdown, now)
