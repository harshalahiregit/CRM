...
export default function TpvComplianceRegister() {
  ...
  // Merge tracked-roster vendors with all vendors (so untracked vendors can be started).
  ...
  const allRows = vendors
    ...
    .map(v => rosterById[v.id] || { vendor_id: v.id, vendor: v.company_name, vendor_code: v.vendor_code, tracked: 0, ok: 0, problems: 0, expiring: 0, percent: 0 })
...
function VendorRow({ r, meta, expanded, onToggle, onChanged }) {
  ...
  return (
    <>
      ...
      {expanded && (
        <tr><td colSpan={6} style={{ padding: '0 14px 14px', background: 'var(--bg-input,rgba(124,58,237,0.03))' }}>
...
function Matrix({ vendorId, statuses, onChanged }) {
  ...
  const load = useCallback(() => { tpvApi.vendorCompliance.matrix(vendorId).then(d => setMatrix(d?.matrix ?? [])).catch(() => setMatrix([])) }, [vendorId])
  useEffect(load, [load])

  const save = async (cat, patch) => {
    const current = matrix.find(m => m.category === cat)
    await tpvApi.vendorCompliance.upsert(vendorId, {
      category: cat,
