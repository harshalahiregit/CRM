...
// Sangoe TPV §25 — generalised CAPA register. Corrective/Preventive actions from
// any governance source (NCR, inspection/audit, meeting, violation, renewal,
// incident) or standalone. Open → In_Progress → Done → Verified; a CAPA cannot
...
export default function TpvCapaRegister() {
  const { user } = useAuth()
  ...
  const advance = async (row, status) => {
    try { await tpvApi.capas.transition(row.id, { status }); load() }
    catch (e) { alert(e?.response?.data?.message || 'Could not update.') }
  }
  const remove = async (id) => { if (window.confirm('Delete this CAPA?')) { await tpvApi.capas.delete(id); load() } }

  const st = meta.stats || {}
  const cards = [
    { k: 'Total', v: st.total ?? 0, c: '#a78bfa' },
...
function CapaModal({ row, meta, vendors, onClose, onSaved }) {
  ...
  const seed = () => {
    const b = {
      vendor_id: row?.vendor_id || '', source_kind: row?.source_kind || 'manual',
  ...
  const save = async () => {
    ...
    try {
      ...
      if (row) await tpvApi.capas.update(row.id, payload)
      else await tpvApi.capas.create(payload)
      onSaved()
...
const miniBtn = { display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-card)', color: '#a78bfa', fontSize: 11.5, fontWeight: 700, marginRight: 4 }
