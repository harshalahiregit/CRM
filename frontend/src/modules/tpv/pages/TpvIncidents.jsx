...
/**
 * HSSE Incidents → RCA → CAPA (Doc_4 Phase 5). Report an incident (a Serious/
 * Fatal or stop-work one auto-suspends the vendor), record its root cause, drive
 * its corrective actions to Verified, then close it — the backend refuses to
 * close until the investigation is complete.
 */
export default function TpvIncidents() {
...
function ReportModal({ onClose, onSaved }) {
  ...
  const [f, setF] = useState({ title: '', type: 'Injury', severity: 'Minor', vendor_id: '', location: '', occurred_at: '', description: '', immediate_action: '', stop_work: false })
  ...
  const willSuspend = (f.severity === 'Serious' || f.severity === 'Fatal' || f.stop_work) && f.vendor_id

  const save = async () => {
    if (!f.title.trim()) { setErr('A title is required.'); return }
    setBusy(true); setErr('')
    try {
      await tpvApi.incidents.create({ ...f, vendor_id: f.vendor_id || null, occurred_at: f.occurred_at || null })
      onSaved()
    ...
    <Overlay onClose={onClose} title="Report Incident">
      ...
      {willSuspend && (
        <div style={{ marginTop: 12, padding: '9px 12px', borderRadius: 9, background: 'rgba(249,115,22,0.12)', border: '1px solid rgba(249,115,22,0.3)', fontSize: 12, color: '#f97316', display: 'flex', alignItems: 'center', gap: 8 }}>
          <AlertTriangle size={14} /> This will auto-suspend the selected vendor — site access is withheld until an admin reinstates them.
        </div>
      )}
...
function DetailModal({ id, onClose, onChanged }) {
  ...
  const saveRca = () => { if (!rca.root_cause.trim()) { setErr('Root cause is required.'); return } act(() => tpvApi.incidents.recordRca(id, rca)) }
  const addCapa = () => { if (!capa.description.trim()) { setErr('CAPA description is required.'); return } act(async () => { await tpvApi.incidents.addCapa(id, { ...capa, due_date: capa.due_date || null }); setCapa({ type: 'Corrective', description: '', due_date: '' }) }) }
  const setCapaStatus = (c, status) => act(() => tpvApi.incidents.updateCapa(id, c.id, { status }))
  const close = () => act(() => tpvApi.incidents.close(id))
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 10 }}>
        {capas.map(c => (
          <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 11px', borderRadius: 9, background: 'var(--bg-input)', border: '1px solid var(--border)' }}>
            ...
            <span style={{ flex: 1, minWidth: 0, fontSize: 12.5, color: 'var(--text-h)' }}>{c.description}{c.due_date ? <span style={{ color: 'var(--text-muted)' }}> · due {c.due_date}</span> : ''}</span>
            {inc.status !== 'Closed' ? (
