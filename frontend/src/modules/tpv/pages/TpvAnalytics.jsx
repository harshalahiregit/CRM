...
// Sangoe TPV §33 — Reports & Analytics. Cross-module posture, month-over-month
// trend series, a per-vendor benchmark leaderboard, and CSV export of every
// governance dataset. Distinct from the periodic DPR/WPR/MCR print report.
const SERIES = [
  { k: 'ncrs', label: 'NCRs', c: '#ef4444' },
  { k: 'capas', label: 'CAPAs', c: '#22c55e' },
  { k: 'violations', label: 'Violations', c: '#f59e0b' },
  { k: 'inspections', label: 'Inspections', c: '#0ea5e9' },
]
...
export default function TpvAnalytics() {
  const [data, setData] = useState(null)
  const [busy, setBusy] = useState('')

  const load = useCallback(() => {
    tpvApi.analytics.get({ months: 6 }).then(setData).catch(() => setData({ error: true }))
  }, [])
  useEffect(() => { load() }, [load])
  ...
  const download = async (dataset) => {
    setBusy(dataset)
    try {
      const blob = await tpvApi.analytics.export(dataset)
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url; a.download = `tpv-${dataset}.csv`
  ...
  const kpis = [
    { k: 'Vendors', v: overview?.portfolio?.total ?? 0, c: '#a78bfa' },
    ...
    <div style={{ padding: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 12.5, margin: '4px 0 0' }}>Cross-module posture, trends and vendor benchmarks — with CSV export.</p>
        </div>
