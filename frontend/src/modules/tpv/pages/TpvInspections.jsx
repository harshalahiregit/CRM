...
import { ClipboardCheck, Plus, RefreshCw, X, ChevronDown, ChevronRight, Trash2, Pencil, FileWarning } from 'lucide-react'
import { tpvApi } from '@/services/tpvApi'
import LoadError from '@/components/ui/LoadError'
import { KIT3D_STYLE as TPV_STYLE } from '@/components/ui/kit3d'

// Sangoe TPV §22 — Inspections & Audits. Plan → Inspect → Finding → Action →
// CAPA/NCR → Verification → Close. Findings can escalate into an NCR.
...
export default function TpvInspections() {
  ...
  const load = useCallback(() => {
    tpvApi.inspections.list().then(d => { setLoadError(null);
      setRows(d?.data ?? [])
  ...
  const remove = async (id) => { if (window.confirm('Delete this inspection and its findings?')) { await tpvApi.inspections.delete(id); load() } }
    ...
    <div style={{ padding: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 12.5, margin: '4px 0 0' }}>Plan → Inspect → Finding → Action → CAPA/NCR → Verify → Close.</p>
        </div>
  ...
  return (
    <>
      <tr style={{ borderTop: '1px solid var(--border)' }}>
        ...
        <td style={{ padding: '10px 14px', fontWeight: 700, color: '#a78bfa' }}>{i.reference}</td>
      ...
      {expanded && (
        <tr><td colSpan={9} style={{ padding: '0 14px 14px', background: 'var(--bg-input,rgba(124,58,237,0.03))' }}>
...
function Findings({ inspection, meta, onChanged }) {
  ...
  const load = useCallback(() => { tpvApi.inspections.get(inspection.id).then(setDetail).catch(() => setDetail({ findings: [] })) }, [inspection.id])
  useEffect(load, [load])
  const [nf, setNf] = useState({ description: '', category: 'Observation', severity: 'Minor', due_date: '' })

  const add = async () => {
    if (!nf.description.trim()) return
    await tpvApi.inspections.addFinding(inspection.id, { ...nf, due_date: nf.due_date || null })
    setNf({ description: '', category: 'Observation', severity: 'Minor', due_date: '' }); load(); onChanged?.()
  }
  const del = async (id) => { await tpvApi.inspections.deleteFinding(id); load(); onChanged?.() }
  const escalate = async (id) => {
    try { await tpvApi.inspections.escalateFinding(id); load() } catch (e) { alert(e?.response?.data?.message || 'Could not escalate.') }
  }
  ...
  return (
    <div style={{ paddingTop: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8 }}><FileWarning size={14} style={{ color: '#a78bfa' }} /><strong style={{ fontSize: 12.5, color: 'var(--text-h)' }}>Findings</strong></div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
        ...
        {(detail.findings || []).map(f => (
          <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 11px', borderRadius: 9, background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            ...
            {f.ncr
              ? <span style={{ fontSize: 11, fontWeight: 700, color: '#a78bfa' }}>→ {f.ncr.reference}</span>
...
function InspModal({ row, meta, vendors, onClose, onSaved }) {
  ...
  const seed = () => {
    const b = { type: row?.type || (meta.types[0] || 'HSE'), vendor_id: row?.vendor_id || '', status: row?.status || 'Planned' }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (row) await tpvApi.inspections.update(row.id, payload)
      else await tpvApi.inspections.create(payload)
      onSaved()
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>
          <label style={lbl}>Type *
            <select value={form.type} onChange={set('type')} style={inp}>
              {(meta.types.length ? meta.types : ['HSE']).map(t => <option key={t} value={t}>{fmt(t)}</option>)}
            </select>
          </label>
