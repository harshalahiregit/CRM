...
// scorecard: three VRS dimensions plus five governance dimensions (quality/NCR,
// CAPA, conduct, inspection, documentation) into a weighted A–E index.
const DIM_LABEL = {
  safety: 'Safety', compliance: 'Compliance', workforce: 'Workforce', quality: 'Quality',
  capa: 'CAPA', conduct: 'Conduct', inspection: 'Inspection', documentation: 'Docs',
}
const BAND_TONE = { A: '#22c55e', B: '#0ea5e9', C: '#f59e0b', D: '#f97316', E: '#ef4444' }
const scoreTone = (s) => (s >= 80 ? '#22c55e' : s >= 60 ? '#f59e0b' : '#ef4444')
const fmt = (s) => String(s || '').replace(/_/g, ' ')

export default function TpvPerformanceIndex() {
    ...
    <div style={{ padding: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 12.5, margin: '4px 0 0' }}>Eight-dimension weighted index (VRS + governance) — worst first, A–E banded.</p>
        </div>
      ...
      {expanded && rows && <ExpandedDetail vendorId={expanded} onClose={() => setExpanded(null)} />}
    </div>
  )
}
...
function ExpandedDetail({ vendorId }) {
  ...
  useEffect(() => { tpvApi.vpi.vendor(vendorId).then(setD).catch(() => setD(null)) }, [vendorId])
  if (!d) return null
  ...
  return (
    <div className="pr-glass" style={{ marginTop: 12, padding: 18, borderRadius: 14 }}>
      ...
      <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '12px 0 0' }}>Weighted per config/vpi.php · VRS band {d.vrs_band || '—'}. The base VRS scorecard is unchanged; this is an additive superset.</p>
    </div>
  )
}
