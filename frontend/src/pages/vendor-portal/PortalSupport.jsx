...
/**
 * In-app Support & Help page for the vendor portal.
 *
 * Replaces the old dead `mailto:support@company.com` link in the sidebar. It
 * renders real, visible content (so it always "responds", unlike a mailto that
 * silently no-ops without a mail client): the vendor's assigned account manager
 * as the primary contact, a general support address, and quick links back into
 * the portal. All data comes from the existing GET /portal/me — no new endpoint.
 */
    ...
    <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '4px 0 0', lineHeight: 1.5 }}>
              We're here to help with onboarding, documents, workforce and compliance questions.
            </p>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
        ...
        <div className="portal-card portal-card-padded">
          ...
          <a
            href={`mailto:${supportEmail}?subject=${encodeURIComponent('Support request — ' + (vendor?.company_name || 'Vendor Portal'))}`}
        ...
        <div className="portal-card portal-card-padded">
          ...
          <QuickLink to="/vendor-portal/onboarding" icon={ClipboardCheck} title="My Onboarding" desc="Check your onboarding status and next steps" />
          <QuickLink to="/vendor-portal/documents" icon={FileText} title="My Documents" desc="Upload or review your statutory documents" />
          <QuickLink to="/vendor-portal/registration" icon={User} title="My Company" desc="View and update your company profile" />
          ...
          <div style={{ marginTop: 14, padding: '12px 14px', background: 'var(--bg-input)', borderRadius: 10, border: '1px solid var(--border)', fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6 }}>
            ...
            {' '}<strong style={{ color: 'var(--text-h)' }}>{vendor?.vendor_code || '—'}</strong>
