...
| 2026-08-27 | `3f909a59` | SangoeTrack integration: 11 HR sections live from track.sangoe.in; plus 52 merged commits of Harshal's TPV/Purchase and Shivam's meetings work | **LIVE.** 19 additive migrations, clean run. Deployed by clone+rsync from `deploy/3f909a59` carrying a prebuilt `dist`. Two traps hit, neither damaging: the paste used a literal `deploy/<SHA>` so the clone silently never ran and both rsyncs reported `sent 19 bytes` — check for that line, it is the signature of a missing source. And the clone root has NO `CRM/` wrapper: paths are `/tmp/sangoe-master/backend/`, not `/tmp/sangoe-master/CRM/backend/`. Also note the backend rsync `--delete` removes `public/icons/`, `manifest.webmanifest`, `favicon.svg` and `sangoe-icon.png`; the frontend rsync restores them, so BACKEND MUST RUN FIRST. Route caching is NOT enabled on track.sangoe.in but IS on app.sangoe.in. Next BASE = `3f909a59`. |
| 2026-08-27 | `ed50df4d` | SangoeTrack nav moved into Sidebar.jsx; LoadError given the raw error | **LIVE.** Frontend only — dist rsync, no `--delete`, no migrations. The nav had gone into HRLayout (the horizontal strip inside HR pages); the sidebar people navigate by is Sidebar.jsx, which keeps its own list. |
| 2026-08-27 | `389578e1` | SangoeTrack client re-authenticates on 403, not just 401 | **LIVE.** One file, copied with `cp` — no rsync, no migrations, no dist. Next BASE = `389578e1`. **`.env` is excluded from every rsync, so config added locally NEVER reaches live** — SANGOETRACK_* had to be added on the server by hand, and the screens failed until it was. Check `.env` on both sides whenever an integration reads config. |
| 2026-08-27 | `467a9b76` | SangoeTrack history: 5 read-only endpoints on their side, Waiting-on-you/History switch on all four approval screens, attendance date navigation | **LIVE.** Backend + frontend, no migrations. **Verification trap:** grepping `assets/index-*.js` for UI text returns 0 for anything inside a lazy-loaded route — Vite code-splits it into its own chunk. Fetch the specific chunk instead, or just confirm a file NEW to that build returns 200. This has now looked like a failed deploy twice when the deploy was fine. Next BASE = `467a9b76`. |
| 2026-08-27 | `22dd159e` | Attendance gains the six columns it was fetching and dropping (early leaving, overtime, break, clock-out selfie, IP, location); CSV export on 7 screens | **LIVE.** Frontend only — dist rsync, no backend, no migrations, no cache commands. Next BASE = `22dd159e`. |
| 2026-08-27 | `c7f0a715` | Write-path audit: reset password, disburse field names, attendance paging, clock-out location, advance statuses | **LIVE.** The reads were verified against live data all along; the WRITES never were, and three of them did not work. Audit request shapes against the other side's validators before assuming a write path functions. |
| 2026-08-27 | `d8130f1c` | Holidays editable from the CRM — 5 endpoints on SangoeTrack plus the screen | **LIVE.** Backend + frontend, no migrations. Import endpoint exists but has no button yet. Next BASE = `d8130f1c`. |
| 2026-08-29 | `94fb2809` | Sidebar: one name (HR, was HRMS in one of two renderers) and one tree (HR removed from the flat pin, which dropped its groups and the SangoeTrack divider) | **LIVE.** Frontend only — dist rsync, no backend, no migrations, no cache commands. Next BASE = `94fb2809`. |
| 2026-08-29 | `27982002` | HR spans the full content width — `AppShell` no longer caps HR pages at 1440px, so the header band reaches the sidebar and the window edge | **LIVE.** Frontend only — dist rsync, no backend, no migrations, no cache commands. Next BASE = `27982002`. **The first attempt was a multi-line python replace that matched nothing:** the new `fullBleed` const landed, the `<div>` it was meant to guard did not, and `vite build` passed because an unused const is legal JS. A green build is not evidence a change applied — grep the source line AND the compiled bundle. |
| 2026-08-29 | `c7bcd4ce` | The hide-amounts eye was rendered twice on desktop — the top bar and four pages both drew it off one shared `MoneyVisibilityContext` | **LIVE.** Frontend only — dist rsync, no backend, no migrations, no cache commands. Next BASE = `c7bcd4ce`. `MoneyToggle` is now `md:hidden`, the exact complement of the header's `hidden md:flex` — fixing all four pages (customer detail, group reports, sales dashboard, accounts dashboard) without editing any of them. The trap to avoid was deleting the four outright: the header copy vanishes below 768px, so that would have cost mobile the feature entirely. Verified in the built CSS that `.md\:hidden` (byte 84838) falls after `.flex` (31095) inside `@media (min-width:768px)`, so the cascade actually resolves as intended at every width including 768 itself. |
| 2026-08-29 | `9f271a7c` | HR no longer needs re-installing from the Modules page on every browser — removed the `isModuleInstalled('hr')` gate | **LIVE.** Frontend only — dist rsync, no backend, no migrations. Next BASE = `9f271a7c`. It was the ONLY `isModuleInstalled` call in the app; the other eleven modules render unconditionally, which is why only HR behaved this way. The gate read localStorage, so HR vanished on a new browser/machine/incognito/colleague's login. It protected nothing — routes and API are ungated, `/app/hr/dashboard` always loaded. Verified by grepping the deployed main bundle for `sangoe_installed_modules` → 0 (the string now lives only in the ModulesPage chunk). |
